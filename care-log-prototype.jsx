...
function Sparkline({ points, kind, unitSystem, scaleMax, meds, domain, onViewChange }) {
  ...
  // Only label points when there aren't too many, so a busy chart doesn't turn into
  // the same overlapping-text problem the medication markers had.
  const showValueLabels = kind === "temperature" && display.length > 0 && display.length <= 8;
  ...
  const valueLabels = showValueLabels
    ? display.map((p) => {
        ...
        lastLabelX = x;
        return { x, y: scaleY(p.v), value: p.v, below: labelBelow };
      })
    : [];
  ...
  return (
    <div>
      <svg
        ...
        {/* Temperature values written next to each point — only while there are few
            enough points to stay readable. A dense chart (many readings in view) skips
            the numbers rather than risk the same overlap/cutoff problems the medication
            labels had; the dots and line are still there, and a narrower range preset
            or a pinch-zoom brings the numbers back. */}
        {valueLabels.map((l, i) => (
          <g key={i}>
            <text
