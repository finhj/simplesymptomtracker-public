...
function Sparkline({ points, kind, unitSystem, scaleMax, meds, treatments, flaggedPeriods, domain, onViewChange }) {
  ...
  // Midnight (local time) boundaries within the visible range, so a new day is
  // visually obvious on a multi-day chart. Capped — past a certain point the lines
  // would sit close enough together to be visual noise rather than a useful cue,
  // so this quietly steps back to relying on the start/end axis labels instead.
  const MAX_DAY_LINES = 10;
  const dayBoundaries = [];
  {
    const firstMidnight = new Date(minX);
    firstMidnight.setHours(24, 0, 0, 0); // the *next* midnight after minX, not minX's own day
    for (let t = firstMidnight.getTime(); t < maxX; t += 24 * 3600 * 1000) {
      dayBoundaries.push(t);
      if (dayBoundaries.length > MAX_DAY_LINES) break;
    }
  }
  const showDayLines = dayBoundaries.length > 0 && dayBoundaries.length <= MAX_DAY_LINES;
  // Alternating background shading between days — every other day segment (using
  // the same boundaries as the dotted lines) gets a faint neutral tint, giving a
  // "zebra stripe" rhythm across the timeline without needing any text labels.
  const daySegments = showDayLines
    ? [minX, ...dayBoundaries, maxX].slice(0, -1).map((t, i, arr) => ({
        x1: scaleX(t),
        x2: scaleX(i + 1 < arr.length ? arr[i + 1] : maxX),
        shaded: i % 2 === 1,
      }))
    : [];
  ...
  return (
    <div>
      <svg
        ...
        {/* Alternating day shading renders first, as the bottom-most layer — a faint
            neutral rhythm marking calendar days, never competing with the fever band,
            flagged periods, or the data itself for visual attention. */}
        {daySegments.map((s, i) =>
          s.shaded ? (
            <rect key={i} x={s.x1} y={pad} width={Math.max(s.x2 - s.x1, 0)} height={height - pad * 2} fill={border} opacity={0.25} />
          ) : null
        )}
        {/* Flagged periods (relapses, flares, etc.) render next, as a background band
        ...
        {/* Dotted lines mark the exact day boundary, on top of the shading but still
            behind the actual data line and points. */}
        {showDayLines && dayBoundaries.map((t) => (
          <line
            key={t}
            x1={scaleX(t)} y1={pad} x2={scaleX(t)} y2={height - pad}
            stroke={inkSoft} strokeWidth={1} strokeDasharray="2 3" opacity={0.6}
          />
        ))}
        {kind === "temperature" && (
          <>
...
function TrackerCard({ tracker, series, latest, history, unitSystem, meds, treatments, flaggedPeriods, domain, onViewChange, draftValue, onDraftChange, onLog, onArchive, onUpdateEntry, onDeleteEntry }) {
  ...
  const lastSummary = latest ? `${formatValue(latest.values[tracker.id])}${unitLabel || ""} · ${fmtTimeShort(latest.timestamp)}` : "No readings yet";
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: collapsed ? 0 : 8 }}>
        ...
        >
          ...
          <div>
            ...
            {collapsed && <div style={{ fontSize: 12, color: inkSoft }}>{lastSummary}</div>}
          </div>
        </button>
...
function MedicationSection({ meds, medName, setMedName, medDose, setMedDose, onAdd, onDelete, onUpdate, catalogCount, knownMedNames, concerns }) {
  ...
  return (
    <div style={{ background: card, border: `1px solid ${border}`, borderRadius: 16, padding: 14, marginTop: 20, marginBottom: 16 }}>
      <button
        onClick={() => setCollapsed((c) => !c)}
        style={{ display: "flex", alignItems: "center", gap: 6, background: "none", border: "none", padding: 0, cursor: "pointer", marginBottom: collapsed ? 0 : 10 }}
      >
        <ChevronLeft size={16} color={inkSoft} style={{ transform: collapsed ? "rotate(-90deg)" : "rotate(-90deg) scaleY(-1)", flexShrink: 0 }} />
        <div style={{ fontSize: 16, fontWeight: 700 }}>Medications{collapsed && meds.length > 0 ? ` (${meds.length})` : ""}</div>
      </button>
      {!collapsed && (
      <>
      <div style={{ display: "flex", gap: 8, marginBottom: 8 }}>
...
function TreatmentSection({
  ...
  return (
    <div style={{ background: card, border: `1px solid ${border}`, borderRadius: 16, padding: 14, marginTop: 20, marginBottom: 16 }}>
      ...
      >
        <ChevronLeft size={16} color={inkSoft} style={{ transform: collapsed ? "rotate(-90deg)" : "rotate(-90deg) scaleY(-1)", flexShrink: 0 }} />
        <div style={{ fontSize: 16, fontWeight: 700 }}>Treatments{collapsed && treatments.length > 0 ? ` (${treatments.length})` : ""}</div>
      </button>
      {!collapsed && (
      <>
      <p style={{ fontSize: 12, color: inkSoft, margin: "0 0 10px" }}>
